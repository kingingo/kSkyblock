...
public class CommandParty implements CommandExecutor{
	...
	public boolean onCommand(CommandSender cs, Command cmd, String arg2,String[] args) {
		p=(Player)cs;
			if(args.length==0){
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_PREFIX"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD1"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD2"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD3"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD4"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD5"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD6"));
				p.sendMessage(TranslationManager.getText(p, "SKYBLOCK_PARTY_CMD7"));
			}else{
				if(args[0].equalsIgnoreCase("erstellen")||args[0].equalsIgnoreCase("create")){
					if(getInstance().getManager().haveIsland(p)){
						...
						if(world.createParty(p)){
							...
							UtilFirework.start(p.getLocation().add(UtilMath.r(4),UtilMath.RandomInt(10, 5),UtilMath.r(4)), UtilFirework.RandomColor(), UtilFirework.RandomType());
							UtilFirework.start(p.getLocation().add(UtilMath.r(4),UtilMath.RandomInt(10, 5),UtilMath.r(4)), UtilFirework.RandomColor(), UtilFirework.RandomType());
							p.sendMessage(TranslationManager.getText(p, "PREFIX")+TranslationManager.getText(p, "SKYBLOCK_PARTY_ERSTELLT"));
						}else{
					...
					if(!b)p.sendMessage(TranslationManager.getText(p, "PREFIX")+TranslationManager.getText(p, "SKYBLOCK_PARTY_EINLADEN_NO"));
					...
					if(world!=null){
						...
						p.sendMessage(TranslationManager.getText(p, "PREFIX")+TranslationManager.getText(p, "SKYBLOCK_PARTY_NO"));
					}
					...
					if(args.length==2){
						...
						p.sendMessage(TranslationManager.getText(p, "PREFIX")+"§6/party kicken [Player]");
					}
				}else{
					p.sendMessage(TranslationManager.getText(p, "PREFIX")+TranslationManager.getText(p, "NO_BEFEHL"));
				}
			}
